...
WHERE id = $1`, userID, targetBalanceKopecks); err != nil {
	...
	slog.Info("finance.balance.set", "user_id", userID, "delta_kopecks", delta, "balance_before_kopecks", current, "balance_after_kopecks", targetBalanceKopecks, "device_accesses_resumed", resumed, "device_accesses_suspended", suspended, "error_code", "E_BILLING_SET")
...
RETURNING balance_kopecks`, userID, next).Scan(&next); err != nil {
	...
	slog.Info("finance.balance.adjusted", "user_id", userID, "operation_type", op, "delta_kopecks", amountKopecks, "balance_before_kopecks", current, "balance_after_kopecks", next, "device_accesses_resumed", resumed, "device_accesses_suspended", suspended, "error_code", "E_BILLING_ADJUST")
	return tx.Commit(ctx)
}
...
func (w DailyChargeWorker) chargeUser(ctx context.Context, userID string, billableDevices int64) error {
	...
	slog.Info("finance.daily_charge.applied", "user_id", userID, "delta_kopecks", -chargeAmount, "billable_devices", billableDevices, "balance_before_kopecks", current, "balance_after_kopecks", next, "device_accesses_resumed", resumed, "device_accesses_suspended", suspended, "error_code", "E_BILLING_DAILY")
	return tx.Commit(ctx)
}
