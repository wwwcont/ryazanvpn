...
func TestNodeAgentAuthMiddleware_RejectsMissingProtocolVersion(t *testing.T) {
	secret := "secret"
	ts := time.Now().UTC().Unix()
	body := "{}"
	sig := auth.Sign([]byte(secret), strconv.FormatInt(ts, 10), []byte(body))
...
func TestNodeAgentAuthMiddleware_RejectsUnsupportedProtocolVersion(t *testing.T) {
	secret := "secret"
	ts := time.Now().UTC().Unix()
	body := "{}"
	sig := auth.Sign([]byte(secret), strconv.FormatInt(ts, 10), []byte(body))
	...
	req.Header.Set(auth.HeaderTimestamp, strconv.FormatInt(ts, 10))
	req.Header.Set(auth.HeaderSignature, sig)
	req.Header.Set(nodeapi.HeaderProtocolVersion, "-1")
	req.Header.Set("X-Node-Id", "n1")
	req.Header.Set("X-Node-Token", "token")
	req.Header.Set("X-Request-Id", "req-1")
	rr := httptest.NewRecorder()
...
func TestNodeAgentAuthMiddleware_AcceptsPreviousProtocolVersion(t *testing.T) {
	secret := "secret"
	ts := time.Now().UTC().Unix()
	body := "{}"
	sig := auth.Sign([]byte(secret), strconv.FormatInt(ts, 10), []byte(body))
	...
	req.Header.Set(auth.HeaderTimestamp, strconv.FormatInt(ts, 10))
	req.Header.Set(auth.HeaderSignature, sig)
	req.Header.Set(nodeapi.HeaderProtocolVersion, nodeapi.PreviousProtocolVersion)
	req.Header.Set("X-Node-Id", "n1")
	req.Header.Set("X-Node-Token", "token")
	req.Header.Set("X-Request-Id", "req-1")
	rr := httptest.NewRecorder()
